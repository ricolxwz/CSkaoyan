...
\subsection{指令}
\noindent {\kaishu \verb|bne|和\verb|beq|指令:}
\begin{itemize}
...
\noindent {\kaishu \verb|slt|和\verb|slti|指令:}
\begin{itemize}
...
\end{enumerate}
\subsubsection{\texttt{IEEE 754}中浮点数的类型}
\begin{table}[H]
\centering
\begin{tabularx}{\textwidth}{|Z|Z|Z|Z|Z|Z|}
\hline
...
\end{itemize}
\subsection{\texttt{SRAM}和\texttt{DRAM}的区别}
\begin{table}[H]
...
\begin{enumerate}
\item 软件查询法
\item 硬件向量法: 由{\heiti 硬件}产生{\heiti 向量地址}, 再由{\heiti 向量地址}找到{\heiti 入口地址(中断向量)}
\end{enumerate}
\end{enumerate}
\begin{figure}[H]
\centering
...
\end{figure}
\subsubsection{中断服务程序}
由中断隐指令, 我们已经得到了中断服务程序的入口地址(中断向量), 现在我们开始执行中断服务程序:
\begin{enumerate}
\item 保护现场: 保护通用寄存器和状态寄存器的内容, 使用栈或者其他存储单元
\item 中断服务: 主体部分, 具体看中断源的请求是什么
\item 恢复现场: 通过出栈指令或取数指令把信息恢复
\item 中断返回: 通过中断返回指令返回到源程序断点处
\end{enumerate}
\begin{figure}[H]
\centering
